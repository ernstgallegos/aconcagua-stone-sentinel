...
- Added dedicated renderer modules `prototype/web-v1/ui/screens/title.js`, `prototype/web-v1/ui/screens/game.js`, `prototype/web-v1/ui/screens/debrief.js`, and `prototype/web-v1/ui/screens/part2.js` to isolate screen-specific rendering paths while keeping `ui/screens.js` as runtime orchestration entrypoint and global facade surface.
...
- Refactored `prototype/web-v1/ui/screens.js` into a thinner orchestrator by delegating screen rendering responsibilities into the new `ui/screens/*` modules via renderer factories; preserved flow-controller authority, game-loop/turn-resolution wiring, and existing `window.*` API contracts.
- Updated `prototype/web-v1/tests/new-mechanics.test.js` to aggregate assertions across `ui/screens.js` plus extracted renderer modules so characterization checks continue to protect copy/hooks after renderer extraction.
