...
- Rebuilt the public landing visual system in `index.html` + `src/styles/public-tokens.css` with a slower editorial alpine layout (asymmetrical hero composition, atmospheric depth layers, restrained warm-accent hierarchy, and expanded reading rhythm), and propagated the same palette/interaction direction to `prototype/web-v1/css/{tokens,layout,components,screens}.css` for title-screen/control coherence.
