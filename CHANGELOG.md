...
- Added `prototype/web-v1/ui/helpers/i18n.js`: extracted all i18n/translation state from `screens.js`. Exports `LANGUAGE_KEY`, `VALID_LANGUAGES`, `CURRENT_LANGUAGE`, `I18N`, `TUTORIAL_CONTENT`, `localizeCharacter`, `localizeScenario`, `t`, `uiText`, and new `setCurrentLanguage` setter. Internal `CHARACTER_I18N` and `SCENARIO_I18N` remain module-private.
- Added `prototype/web-v1/ui/helpers/route-data.js`: extracted route-related mutable state from `screens.js`. Exports `ROUTE_NODES`, `POSITIONS`, `POS_LABELS`, `POS_ALT`, `POS_BAND`, `CAMP_POSITIONS`, `STAGE_BY_POSITION`, `CANONICAL_OUTCOMES`, `rebuildRouteData(dataConfig)`, `getCurrentStage`, and `isCampPosition`.
- Added `prototype/web-v1/ui/helpers/difficulty.js`: extracted difficulty configuration from `screens.js`. Exports `DIFFICULTY_LEVELS`, `CURRENT_DIFFICULTY_ID`, `DEFAULT_DIFFICULTY_ID`, `getDifficultyConfig`, `getDifficultyModifiers`, `difficultyLabel`, and `setCurrentDifficultyId`.
- Added `getCharacterImagePath` export to `prototype/web-v1/ui/helpers/carousel-media.js`: maps character IDs to portrait asset paths with optional Part 2 variant support.
- Added `metricDisplay` export to `prototype/web-v1/ui/helpers/screen-utils.js`: returns a display string for a metric value based on confidence tier (high → value, med → `?`, low → `??`).
...
- Added `prototype/web-v1/ui/screens/game.js`: extracted game screen rendering functions from `screens.js` into a dedicated module. Contains `makeDots`, `clearElement`, `bodyValueClass`, `capacityLabel`, `fatigueLabel`, `exposureLabel`, `updateTurnProgress`, `setMetricValue`, `updatePermitWidget`, `getOnboardingLayer`, `getRiskProfile`, `renderContextWidget`, `renderWatch`, `renderPositionList`, `syncMobileStatusPanels`, `NARRATIVES`, `NARRATIVES_ES`, `pickNarrative`, `renderNarrative`, `updateAmbientSignal`, `maybeShowTutorial`, and `addLogEntry`. Dependencies on `screens.js`-local functions (`getSimConfig`, `getDecisionPressureCopy`, `canUseShootPhoto`) are injected via `initGameScreen(deps)` to avoid circular imports.
- Added `prototype/web-v1/ui/screens/title.js`: extracted title screen rendering from `screens.js`. Contains `renderIntroContent`, `renderTutorialContent`, `renderDifficultySelector`, `updateSocialShareLinks`, `copyProjectShareLink`, `getProjectShareUrl`, `setDifficulty`, `initDifficulty`, and `initWelcomeScreen`.
- Added `prototype/web-v1/ui/screens/debrief.js`: extracted debrief screen rendering from `screens.js`. Contains `classifyOutcome`, `findTurningPoint`, `findPrimaryCause`, `classifyDifficultyResponsibility`, `buildDebriefAnalytics`, `updateDebriefHero`, `updateRunReviewPanel`, `reviewPrevTurn`, `reviewNextTurn`, `copyRunSignature`, `exportRunLog`, `buildRunLogExport`, `buildMockTurnLog`, and `buildReflectionPrompts`.
- Added `prototype/web-v1/ui/screens/part2.js`: extracted Part 2 setup and narrative rendering from `screens.js`. Contains `CAROUSEL_STATE_PART2`, `PART2_ROUTE_OPTIONS`, the full `PART2_NARRATIVE_SEQUENCE` (EN + ES), `renderPart2Carousel`, `togglePart2CarouselInfo`, `buildPart2SetupScreen`, `updatePart2ConfirmState`, `handlePart2NarrativeAction`, `renderPart2NarrativeScreen`, `confirmPart2Character`, and related helpers. Initialized via `initPart2Screen({ getCharacters })` to avoid circular DATA_CONFIG imports.
- Added `DIFFICULTY_STORAGE_KEY` export to `prototype/web-v1/ui/helpers/difficulty.js`.
...
- Refactored `prototype/web-v1/ui/screens.js` to import and re-export all extracted rendering functions from `ui/screens/title.js`, `ui/screens/game.js`, `ui/screens/debrief.js`, `ui/screens/part2.js`, `ui/helpers/i18n.js`, `ui/helpers/route-data.js`, and `ui/helpers/difficulty.js`. Removed ~1,650 lines of rendering code from `screens.js`; remaining orchestration calls `initGameScreen` and `initPart2Screen` at startup to inject DATA_CONFIG-dependent closures. `setLanguage()` now calls `setCurrentLanguage()` instead of directly mutating the imported live binding. `loadDataConfig()` now calls `rebuildRouteDataHelper(DATA_CONFIG)` after config load. No gameplay behavior change.
- Updated `prototype/web-v1/tests/new-mechanics.test.js` and `prototype/web-v1/tests/parity/i18n-static-coverage.test.js` to check extracted module files (`difficulty.js`, `game.js`, `part2.js`) for content that moved out of `screens.js`.
