...
### Changed
- Redesigned the public landing hero into a full-bleed brand-first composition with cinematic typography, reduced first-view clutter, and atmospheric motion tuned for desktop/mobile parity.
- Refreshed `web-v1` welcome/title presentation with explicit project identity copy, updated alpine palette tokens, and intentional motion hierarchy (brand reveal + CTA pulse) while preserving existing flow hooks.
