...
- Enhanced mountain visualization module (`prototype/web-v1/ui/helpers/mountain-visualization.js`) with seven new cinematic subsystems — same public API signatures, backward-compatible `options` extensions, zero new dependencies, `reducedMotion`-safe:
  - **Post-decision transition system**: `TransitionManager` class with `TransitionEffect` primitives. `updateClimberPosition` now accepts `options.action` (`advance`/`advance_slowly`/`descend`/`wait`/`sleep`) triggering camera zoom-push/pull, wind spikes, fog clearing, cloud time-lapse, and sleep dim-then-brighten. `options.flags` array (`white-wind-hit`/`collapse`) drives screen shake and red-edge flash. `options.fatigue`/`options.exposure` > 70 activate progressive dark/blue vignette overlays. Camera class extended with `zoomOffset`, `shakeX`, `shakeY` fields.
  ...
  - **Smooth weather transition**: `lerpAtmosphere` interpolates `_currentAtmosphere` toward `_targetAtmosphere` each frame so weather changes feel gradual. Lightning flash for `weatherSeverity ≥ 3` (~5% chance/frame, 0.1s white sky). Whiteout overlay for `visibility ≤ 1` (white-blue veil with radial silhouette cutout around climber).
  - **Climber pose system**: action-driven stances (advance: 8° forward lean + extended pole/axe; descend: backward lean, wider stance, pole-behind brake; wait: upright, pole resting at side; sleep: sleeping-bag sprite replaces standing figure). Idle micro-animations: head tilt looking at summit every 5s, pack-strap fidget every 8s. Fatigue expression: progressive forward hunch, irregular walk cycle (limp offset), decreased arm swing, asymmetric body bob as `fatigueLevel` increases 0→100.
  - **Cinematic altitude HUD**: vertical altitude gauge (left edge, 70% canvas height) with color zones (green/amber/red), camp tick marks, smooth position dot, highest-reached triangle. Info overlay (bottom-left): altitude, HH:MM clock, weather severity dots (1–4), trend arrow. Summit distance arc (canvas top) showing route progress fraction.
  - **Procedural terrain texture**: rock fracture lines (altNorm 0.25–0.75, 2–3 per strip), scree dots (altNorm 0.35–0.65, 3–5 tiny dots), snow sastrugi curves in snow-covered areas, 1px inter-strip shadow line for depth. All gated behind `fogAmount < 0.5`.
  - **Visual sound cues**: horizontal wind intensity streaks at `windStrength > 1.5` (full canvas width), pulsing pressure ring around climber scaling with `weatherSeverity`, camp marker wind shake oscillation at `windStrength > 2`.
...
- Converted the landing top-left brand label (`Aconcagua: Stone Sentinel`) into a clickable in-page link back to the hero/start section for faster return-to-top navigation while scrolling.
...
- Landing page CSS (`index.html`): completely rewritten inline `<style>` with animated aurora background glow, CSS snow particle layer, Ken Burns slow-zoom hero image, multi-layer vignette fog, gradient text fill (ice-to-amber) on hero title with drop-shadow glow, glassmorphism cards with 3D perspective tilt on hover, mountain ridge SVG silhouette dividers between art strips, shimmer animation on stat values, floating elevation marks, draw-line animation on section kickers, enhanced glassmorphism navigation with deeper blur/saturation/glow, immersive footer with layered mountain ridge silhouettes and ambient radial glow, animated gradient buttons with pulse halo, ice-blue underline animation on footer links, staggered art strip panels for cinema feel, unique ambient undertones per section, and decorative pull-quote quotation marks. Full `prefers-reduced-motion` coverage for all new effects.
- Shared design tokens (`src/styles/public-tokens.css`): expanded palette with new background layers (`--bg-3`), surface tiers (`--surface-3`), border variants (`--border-medium`, `--border-glow`), text tier (`--text-faint`), accent colors (`--accent-ice-bright`, `--accent-amber-warm`, `--accent-rose`, `--accent-aurora`, `--accent-summit`), glow shadows (`--shadow-glow-ice`, `--shadow-glow-amber`, `--shadow-inset`), spacing (`--space-8`), radius (`--radius-xl`), animation easing curves (`--ease-out-expo`, `--ease-out-quart`, `--ease-in-out-sine`), and animation durations (`--dur-micro` through `--dur-long`).
- Prototype tokens (`prototype/web-v1/css/tokens.css`): aligned with new landing palette — deeper background/surface values, new border-glow token, ice-bright/accent-rose/accent-aurora colors, radius-lg, enhanced easing and shadow tokens.
- Prototype components (`prototype/web-v1/css/components.css`): buttons and cards upgraded with glassmorphism (backdrop-filter blur, frosted-glass backgrounds, glow shadow on hover), primary button with animated gradient sweep and enhanced hover glow, ghost button with backdrop blur and inset shine on hover.
- Prototype animations (`prototype/web-v1/css/animations.css`): added shimmer, float-gentle, ice-pulse, and ambient-glow keyframes; refined screen-enter/exit with subtle scale.
- Prototype layout (`prototype/web-v1/css/layout.css`): panel-shell gains backdrop-filter blur and inset shadow for depth.
- Prototype themes (`prototype/web-v1/css/themes.css`): sunset theme palette refined with surface3, border-glow, ice-bright, accent-rose, accent-aurora tokens.
