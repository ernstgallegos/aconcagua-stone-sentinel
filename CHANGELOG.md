...
- Redesigned the public landing page (`index.html`) with a new distinctive visual direction (Fraunces + Syne typography, obsidian/copper/glacial palette, cinematic texture layers, stronger motion hierarchy, and an image-rich “Visual atlas” section) to replace the prior generic look while preserving bilingual behavior and core information architecture.
- Re-themed `prototype/web-v1` presentation to match the landing redesign by updating shared typography and palette tokens (`Fraunces`/`Syne`, copper + glacial accent system), background atmosphere, language-switch accents, and high-impact UI animation timing/glow so both public surfaces feel part of one coherent design system.
