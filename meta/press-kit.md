# Aconcagua: Stone Sentinel — Press Kit

**Public document · April 2026**  
**Version:** 1.2  
**Status:** Public web prototype v1.5.2  
**Spanish version:** [`meta/press-kit.es.md`](./press-kit.es.md)

---

## One-sentence pitch

*Aconcagua: Stone Sentinel* is a contemplative indie game about reading signals, respecting limits, and making decisions that do not rewind — set on the highest peak outside Asia.

---

## Short description (1 paragraph)

*Aconcagua: Stone Sentinel* is a mountain-first narrative systems game developed as an independent project in Argentina. Players take on one of six expedition roles and attempt to navigate Aconcagua's environmental pressures, route constraints, and physiological realities in a turn-based structure built on a real Environmental Pressure / Body Tolerance model. The summit is one valid outcome among many — safe return, strategic retreat, and permit management carry equal narrative weight. The public web prototype (Part 1) is currently playable at [aconcaguastonesentinel.com](https://aconcaguastonesentinel.com).

---

## Key facts

| | |
|---|---|
| **Project name** | Aconcagua: Stone Sentinel |
| **Developer** | Independent / solo (Argentina) |
| **Platform** | Web (public prototype); target platforms TBD |
| **Genre** | Contemplative narrative / systems-driven |
| **Current release** | v1.5.2 — public web prototype (Part 1) |
| **Playable prototype** | [aconcaguastonesentinel.com](https://aconcaguastonesentinel.com) |
| **Repository** | [github.com/ernstgallegos/aconcagua-stone-sentinel](https://github.com/ernstgallegos/aconcagua-stone-sentinel) |
| **Press contact** | aconcaguastonesentinel@gmail.com |
| **Instagram** | [@aconcaguastonesentinel](https://www.instagram.com/aconcaguastonesentinel/) |
| **Languages** | English / Spanish |
| **Origin** | Based on real expedition experience (successful 360 Route ascent, January 2026) |

---

## Design pillars

**1. Environmental authority**  
Altitude, weather, route timing, and fatigue load shape every run. The mountain writes the rules; the player responds.

**2. Partial information**  
Signals are interpreted through confidence ranges, not omniscient dashboards. Perception degrades before collapse is visible.

**3. Plural outcomes**  
Summit-only success is an anti-design choice. Strategic retreat and safe return are valid endings with real mechanical and narrative weight.

**4. Contemplation as gameplay**  
Waiting, resting, observing, and choosing not to act are real actions with real consequences.

---

## What the game is not

- Not an action game set in the mountains.
- Not a survival game built around constant failure and punishment.
- Not a "soulslike" or "boss-fight" interpretation of mountaineering.
- Not empty-postcard tourism: the mountain is a system, not a backdrop.

---

## The characters (v1.5.2)

Six playable roles, each a distinct way of being in the mountains — and a distinct way of misreading it. Every character has a unique engine profile that shapes how they perceive signals, manage resources, and respond to pressure.

### Quick reference

| Character | Nationality | Age | Profession | Difficulty |
|---|---|---|---|---|
| Francisco Aguirre 🇦🇷 | Argentina | 33 | Teacher & amateur endurance runner | Standard |
| Laura Kim 🇰🇷 | South Korea | 38 | Emergency physician | Favorable |
| Erik Lundvall 🇳🇴 | Norway | 45 | Offshore engineer | Demanding |
| Daniela De Rossi 🇮🇹 | Italy | 26 | Photographer & journalist | Demanding |
| Irina Orlova 🇷🇺 | Russia | 41 | Former professional climber | Demanding |
| Blake Harris 🇺🇸 | United States | 58 | Executive | Very Demanding |

---

### Francisco Aguirre 🇦🇷

| Part 1 | Part 2 |
|---|---|
| ![Francisco Aguirre — Part 1](/art/characters/francisco-aguirre.png) | ![Francisco Aguirre — Part 2](/art/characters/part-2/francisco-aguirre.png) |

**Argentina · 33 · Teacher & amateur endurance runner · Difficulty: Standard**

Francisco arrives carrying unresolved grief for Mateo — an older friend who died from COVID-19 in 2021. His playable identity is persistence under pressure; his psychological risk is confusing endurance with meaning. He can sustain effort longer than almost anyone in the group, but his body warnings arrive late and he is prone to interpreting data and effort as mandate.

**Mechanical identity:** High fatigue resistance. Slightly optimistic signal bias (early warnings muted). Solid resource efficiency. Average acclimatization pace. The player who keeps going when rest would be the wiser call.

---

### Laura Kim 🇰🇷

| Part 1 | Part 2 |
|---|---|
| ![Laura Kim — Part 1](/art/characters/laura-kim.png) | ![Laura Kim — Part 2](/art/characters/part-2/laura-kim.png) |

**South Korea · 38 · Emergency physician · Difficulty: Favorable**

Laura reads body and group signals with clinical precision honed in pandemic triage. Her wound is moral fatigue from impossible resource decisions under crisis. Her pressure-point is over-delaying action in pursuit of certainty — a pattern that protected lives in the ICU but can cost critical hours at altitude.

**Mechanical identity:** Best signal clarity and confidence stability in the roster. High resource efficiency and acclimatization rate. Low risk tolerance — she validates carefully before committing. Favorable difficulty because her information quality is consistently superior to the mountain's noise.

---

### Erik Lundvall 🇳🇴

| Part 1 | Part 2 |
|---|---|
| ![Erik Lundvall — Part 1](/art/characters/erik-lundvall.png) | ![Erik Lundvall — Part 2](/art/characters/part-2/erik-lundvall.png) |

**Norway · 45 · Offshore engineer · Difficulty: Demanding**

Erik has strong technical execution and is working through the Seven Summits. Structured and competent, his challenge is ego-filtered risk interpretation: when mountain evidence conflicts with his self-image, he pushes too long. Late risk-signal activation under pressure is his defining mechanical risk.

**Mechanical identity:** High functional capacity and acclimatization rate. Strong resource efficiency. Very high risk tolerance. High perception bias — sees conditions significantly rosier than they are. Demanding because the gap between his felt competence and the mountain's actual state can stay invisible until it is too late.

---

### Daniela De Rossi 🇮🇹

| Part 1 | Part 2 |
|---|---|
| ![Daniela De Rossi — Part 1](/art/characters/daniela-de-rossi.png) | ![Daniela De Rossi — Part 2](/art/characters/part-2/daniela-de-rossi.png) |

**Italy · 26 · Photographer & journalist · Difficulty: Demanding**

Daniela is the sharpest environmental reader in the group — she spots weather shifts, terrain texture, and subtle group dynamics before anyone else. Her challenge is that seeing more does not mean enduring more: her acclimatization is the weakest in the roster, and observation actions cost critical body reserves at altitude.

**Mechanical identity:** Best perception accuracy in the game — negative signal bias means she reads the mountain closer to its actual state. Unique *shoot photo* action (up to 4 uses per run) that improves signal confidence. Lowest acclimatization rate. Low starting functional capacity. Demanding because her body does not keep pace with her perception.

---

### Irina Orlova 🇷🇺

| Part 1 | Part 2 |
|---|---|
| ![Irina Orlova — Part 1](/art/characters/irina-orlova.png) | ![Irina Orlova — Part 2](/art/characters/part-2/irina-orlova.png) |

**Russia · 41 · Former professional climber · Difficulty: Demanding**

Irina returns to high altitude after years away from elite climbing. Her technical memory and movement economy are the best in the roster. Her subtle challenge is pattern-lock: prior protocols from high-performance contexts can overfit present conditions, increasing perceptual noise in genuinely novel situations.

**Mechanical identity:** Highest functional capacity bonus and acclimatization rate. Best resource efficiency. But also the highest risk tolerance — and among the highest perception biases in the group. Elite mechanics with overconfidence as the hidden cost. Demanding because her strongest asset (experience) is also her primary failure mode.

---

### Blake Harris 🇺🇸

| Part 1 | Part 2 |
|---|---|
| ![Blake Harris — Part 1](/art/characters/blake-harris.png) | ![Blake Harris — Part 2](/art/characters/part-2/blake-harris.png) |

**United States · 58 · Executive · Difficulty: Very Demanding**

Blake is built on visible achievement and external validation. He arrives determined to prove he still can, carrying the largest confidence-versus-preparation gap in the group. The mountain denies status shortcuts: logistics, fatigue load, and vulnerability are non-negotiable.

**Mechanical identity:** Lowest starting functional capacity. Poor acclimatization rate and resource efficiency. Highest perception bias — the furthest from accurate signal reading. Very high risk tolerance. The game's hardest character because the gap between his conviction and his body's state is the widest, and it compounds under pressure.

---

## Interpersonal dynamics

The six characters are designed as an ensemble, not just six solo paths. These tensions are canonical for story framing and expedition dynamics:

- **Francisco ↔ Laura:** shared guilt-adjacent burden; the expedition's most functional trust pair when both are reading clearly.
- **Erik ↔ Blake:** validation tension — technical competence filtering versus status-based overcompensation.
- **Irina ↔ Daniela:** expertise distrust versus observational insight; their dynamic resolves when observation proves mountain-first, not aesthetic.

---

## The system (EP/BT model)

The game runs on a turn-resolver where:

1. The player chooses an action: **advance**, **advance slowly**, **wait**, **descend**, **sleep**, or (for Daniela) **shoot photo**.
2. **Environmental Pressure (EP)** is calculated from altitude band, weather severity, terrain load, time of day, and cumulative exposure.
3. **Body Tolerance (BT)** integrates fatigue, exposure accumulation, and functional capacity.
4. A **perceived signal** is generated from EP and BT through a character-specific confidence and trend layer — players never see raw values, only interpreted readings with varying reliability.
5. The outcome is classified: progress, hold, retreat, or terminal event.

All parameters are data-driven (JSON files) and all claims are testable. The engine is fully decoupled from the UI.

---

## The scenarios (v1.5.2)

Five distinct scenarios shape starting conditions, pressure bias, and which danger axis dominates each run:

| # | Name | Difficulty | Core tension |
|---|---|---|---|
| 01 | **Assisted Route** | Low | Learning to read signals before conditions stop being forgiving |
| 02 | **Narrow Weather Window** | Medium | Racing a deteriorating atmospheric trend |
| 03 | **False Stability Terrain** | Medium | Clear skies hiding progressive terrain load buildup |
| 04 | **Accumulated Fatigue Trap** | Medium | Benign conditions masking compounding physical debt |
| 05 | **Weather Window** | Hard | A brief clearing in severe weather — recognize when it opens and closes |

Scenarios share the same route nodes and outcome system. They differ in initial state (fatigue, resources, weather severity) and environmental bias (whether conditions tend to deteriorate, stabilize, or briefly improve).

---

## The outcomes (canonical)

The game has ten terminal outcomes. All are valid; none are exclusively "failure."

| Outcome | What it means |
|---|---|
| **Summit and Safe Return** | Reached the summit and descended to park exit |
| **High Point Return** | Turned back from altitude without incident; the mountain sets the limit |
| **Strategic Retreat** | Deliberate retreat before conditions became critical — a correct read |
| **Rescue** | Deterioration required external intervention |
| **Collapse (Fatigue)** | Physical fatigue exceeded body tolerance threshold |
| **Collapse (Exposure)** | Cumulative environmental exposure exceeded body tolerance |
| **Resource Exhaustion** | Water or food depleted; descent could not be sustained |
| **Expedition Window Closed** | Permit days expired before exit |
| **Permit Expired** | Calendar deadline reached before park exit |
| **Fatality** | Maximum pressure threshold crossed with no recovery margin |

Summit and Safe Return is the rarest outcome — typically 10–30% for human players who internalize the EP/BT system. Strategic Retreat is the most common non-collapse outcome and is explicitly designed to feel like a win.

---

## Technical transparency

- **381 automated tests** covering engine, UI helpers, API, and contract parity.
- **Monte Carlo simulation**: 1,500 headless runs validate structural integrity. No 0% summit rates across any character.
- **Open source**: full repository is public and auditable.
- **5 scenarios**: Assisted Route, Narrow Weather Window, False Stability Terrain, Accumulated Fatigue Trap, Weather Window.
- **10 route nodes** across 3 stages: Approach, High Camp, Summit Day.

---

## Cultural commitment

Aconcagua is not only a record to break. It is a territory layered with Andean cultural presence, ancestral routes, and Indigenous cosmological significance. The project takes a clear stance: no empty folklore, no ornamental local color. Human presence appears through traces, context, and carefully researched references — or not at all.

---

## On AI use

We want to say this clearly.

In this first public stage of *Aconcagua: Stone Sentinel*, we used AI to draft some documents, assist part of the programming work, and create the illustrations currently in this kit.

Not because it is our ideal. Because it was the only way we found to launch the project quickly, with no budget and almost nothing beyond an idea, time, and work.

At this stage, AI was a starting tool. Scaffolding. A provisional prosthesis.

But it is not the horizon we want.

We would love for this first visual identity to be replaced as soon as possible by the work of real human artists and collaborators. We will always prefer the work of people over what a machine can produce.

If this project resonates with you and you feel you could contribute from anywhere in the world, we'd love to hear from you.

This is how we were able to begin.  
Hopefully now it helps us meet real people.

---

## Current state and roadmap

| Stage | Status |
|---|---|
| Part 1 — Web prototype (v1.5.2) | ✅ Public, playable |
| Part 2 — Narrative bridge | Preview unlocked after safe return; full Part 2 in development |
| Stage 7 — Direction evaluation | Planned (next phase) |
| Production platform(s) | TBD after direction lock |

See the public roadmap: [aconcaguastonesentinel.com](https://aconcaguastonesentinel.com) → Waypoints → Public Roadmap.

---

## Visual assets

All visual assets are available in the public repository.

### Character portraits

Six character portraits at `/art/characters/`. Part 2 variants at `/art/characters/part-2/`.

### Concept art

Curated selection from the project gallery at `/art/concept-art/curated/ig/`:

| | | |
|---|---|---|
| ![Scene 01](/art/concept-art/curated/ig/5.png) | ![Scene 02](/art/concept-art/curated/ig/7.png) | ![Scene 03](/art/concept-art/curated/ig/9.png) |
| ![Scene 04](/art/concept-art/curated/ig/11.png) | ![Scene 05](/art/concept-art/curated/ig/13.png) | ![Scene 06](/art/concept-art/curated/ig/17.png) |
| ![Scene 07](/art/concept-art/curated/ig/21.png) | ![Scene 08](/art/concept-art/curated/ig/23.png) | ![Scene 09](/art/concept-art/curated/ig/25.png) |

### Brand

Logo and favicon at `/art/brand/`.

**Usage guidelines:** All visual assets are © 2026 Aconcagua: Stone Sentinel. Press use (editorial, reviews, features) is permitted with attribution. Commercial use requires written permission.

---

## Media contact

**Email:** aconcaguastonesentinel@gmail.com  
**Instagram:** [@aconcaguastonesentinel](https://www.instagram.com/aconcaguastonesentinel/)  
**Repository:** [github.com/ernstgallegos/aconcagua-stone-sentinel](https://github.com/ernstgallegos/aconcagua-stone-sentinel)

We respond to press inquiries, collaboration proposals, and coverage requests. Response time: 3–5 business days.

---

*Press kit version 1.2 · April 2026 · aconcaguastonesentinel.com*


## Launch coordination references

- Launch handoff: [`meta/launch-readiness-handoff.md`](./launch-readiness-handoff.md)
- Stage 7 decision support: [`meta/stage-7-decision-support.md`](./stage-7-decision-support.md)
