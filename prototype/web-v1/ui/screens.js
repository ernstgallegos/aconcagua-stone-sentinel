...
import {
  ...
  buildRandomCharacterCardHtml,
  buildScenarioCardHtml,
  buildRandomScenarioCardHtml,
  buildRouteCardHtml,
  toggleInfoPanel,
  hideInfoPanel,
  stepCarouselIndex,
...
import {
  NARRATIVES,
  NARRATIVES_ES,
  pickNarrative as pickNarrativeHelper,
  resolveNarrativeText,
} from './helpers/narrative.js';
  ...
  reportRuntimeDiagnostic(
    {
      ...
      file: message,
      detail: detail == null ? 'No detail provided.' : String(detail),
    },
...
function setModelLoadError(errorMessage) {
  ...
  reportRuntimeDiagnostic(
    {
      category: errorMessage?.category || 'startup failure',
      file: errorMessage?.file || 'runtime model contract',
      detail: rendered.detail,
    },
    { context: 'startup', level: 'error' }
  );
...
const I18N = {
  en: {
    ...
    ui: {
      ...
      introVersionValue: 'Prototype · v1.5.1',
  ...
  es: {
    ...
    ui: {
      ...
      introVersionValue: 'Prototipo · v1.5.1',
...
function carouselPrev(type) {
  ...
  CAROUSEL_STATE[type].index = stepCarouselIndex(CAROUSEL_STATE[type].index, items.length, 'prev');
  renderCarousel(type);
}
...
function carouselNext(type) {
  const items = getCarouselItems(type);
  if (!items.length) return;
  CAROUSEL_STATE[type].index = stepCarouselIndex(CAROUSEL_STATE[type].index, items.length, 'next');
  renderCarousel(type);
}
...
function part2CarouselPrev(type) {
  ...
  if (!items.length) return;
  CAROUSEL_STATE_PART2[type].index = stepCarouselIndex(CAROUSEL_STATE_PART2[type].index, items.length, 'prev');
...
function part2CarouselNext(type) {
  const items = getPart2CarouselItems(type);
  if (!items.length) return;
  CAROUSEL_STATE_PART2[type].index = stepCarouselIndex(CAROUSEL_STATE_PART2[type].index, items.length, 'next');
  renderPart2Carousel(type);
}

// getCharacterImagePath imported from ./helpers/carousel.js
...
function renderCarousel(type) {
  ...
  if (type === 'character') {
    if (item._random) {
      ...
      cardEl.innerHTML = buildRandomCharacterCardHtml({
        imgPath,
        name: t('ui.randomCharacter'),
        role: t('ui.randomCharacterRole'),
        difficultyLabel: t('ui.charDifficultyLabel'),
        portraitFallback: uiText('Portrait unavailable', 'Retrato no disponible'),
      });
    } else {
      const c = localizeCharacter(item);
      const safeIdx = Number(idx);
      const imgPath = getCharacterImagePath(item.id);
      cardEl.innerHTML = buildCharacterCardHtml({
        character: c,
        imgPath,
        eager: idx === 0,
        difficultyLabel: t('ui.charDifficultyLabel'),
    ...
    hideInfoPanel('carousel-info-panel-character');
  } else if (type === 'scenario') {
    if (item._random) {
      cardEl.innerHTML = buildRandomScenarioCardHtml({
        tag: t('ui.randomScenarioTag'),
        name: t('ui.randomScenario'),
        desc: t('ui.randomScenarioDesc'),
      });
    } else {
      const sc = localizeScenario(item);
      const safeIdx = Number(idx);
      cardEl.innerHTML = buildScenarioCardHtml({
        scenario: sc,
    ...
    hideInfoPanel('carousel-info-panel-scenario');
  }

  renderCarouselDots(dotsEl, items.length, idx);
}
...
function toggleCarouselInfo(type, idx) {
  ...
  toggleInfoPanel({
    panelId: `carousel-info-panel-${type}`,
    idx,
    item,
    buildContent: (it) => {
      if (type === 'character' && !it._random) {
        const c = localizeCharacter(it);
        return `
      ...
      if (type === 'scenario' && !it._random) {
        const sc = localizeScenario(it);
        return `
          <div class="carousel-info-content">
...
function renderPart2Carousel(type) {
  ...
  if (type === 'character') {
    ...
    const safeIdx = idx;
    cardEl.className = `carousel-card${isLocked ? ' part2-locked' : ''}`;
    cardEl.innerHTML = buildRouteCardHtml({
      route: item,
      infoAriaLabel: t('ui.carouselScenInfo'),
      locked: isLocked,
  ...
  hideInfoPanel(`part2-carousel-info-${type}`);
  renderCarouselDots(dotsEl, items.length, idx);
...
function togglePart2CarouselInfo(type, idx) {
  ...
  toggleInfoPanel({
    panelId: `part2-carousel-info-${type}`,
    idx,
    item,
    buildContent: (it) => {
      if (type === 'character') {
      ...
      if (type === 'route') {
        return `
          <div class="carousel-info-content">
            <p class="carousel-info-bio">${it.desc}</p>
            <p class="carousel-info-bio">${isLocked
...
// ════════════════════════════════════════════════
// NARRATIVE — delegated to helpers/narrative.js
// ════════════════════════════════════════════════
