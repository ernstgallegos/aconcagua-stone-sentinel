...
function getStartupI18n() {
  return {
    loading: uiText('Loading expedition model…', 'Cargando modelo de expedición…'),
    ready: uiText('Model loaded. You can begin.', 'Modelo cargado. Puedes comenzar.'),
    unavailable: uiText('Model unavailable.', 'Modelo no disponible.'),
    blockingTitle: uiText('Blocking data error', 'Error bloqueante de datos'),
    blockingSummary: uiText(
      'The simulation model could not be initialized. Gameplay is unavailable until required data is fixed.',
      'No se pudo inicializar el modelo de simulación. La partida no está disponible hasta corregir los datos requeridos.'
    ),
    noDiagnostics: uiText('No diagnostic payload available.', 'No hay diagnóstico disponible.'),
    initTitle: uiText('Blocking model initialization error', 'Error bloqueante de inicialización del modelo'),
    loadFailureSummary: uiText('Required file could not be loaded.', 'No se pudo cargar un archivo requerido.'),
    invalidShapeSummary: uiText('Required file was loaded but failed contract shape checks.', 'Se cargó un archivo requerido pero falló la validación de estructura del contrato.'),
    postLoadSummary: uiText('Files loaded, but cross-file validation failed.', 'Los archivos cargaron, pero falló la validación cruzada.'),
    genericInitSummary: uiText('Model initialization failed before gameplay could start.', 'La inicialización del modelo falló antes de iniciar la partida.'),
    categoryLabel: uiText('Category', 'Categoría'),
    sourceLabel: uiText('Source', 'Origen'),
    detailLabel: uiText('Detail', 'Detalle'),
  };
}
...
function setModelLoadError(errorMessage) {
  ...
  const rendered = renderBlockingError(errorMessage, getStartupI18n());
  console.error(rendered.detail);
  setStartupState('error', uiText('Model unavailable. Review blocking diagnostics.', 'Modelo no disponible. Revisa el diagnóstico bloqueante.'), getStartupI18n());
...
function setLanguage(lang) {
  ...
  renderTutorialContent();
  if (!G.modelReady) {
    setStartupState('error', uiText('Model is still loading or blocked.', 'El modelo aún está cargando o bloqueado.'), getStartupI18n());
  } else {
    setStartupState('ready', '', getStartupI18n());
  }
  if (DATA_CONFIG_ERROR) {
    renderBlockingError(DATA_CONFIG_ERROR, getStartupI18n());
  }
...
async function loadDataConfig() {
  setStartupState('loading', '', getStartupI18n());
