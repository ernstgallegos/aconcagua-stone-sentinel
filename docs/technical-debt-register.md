...
| Residual UI coupling in `screens.js` (title, game widget, and Part 2 narrative renders are delegated to `ui/screens/` modules but `screens.js` still holds partial render logic for carousels and setup screens) | Web-v1 maintainers | Medium: cascade risk when carousel or onboarding UI changes; targeted testing of setup screen logic still requires loading the full `screens.js` module | TBD | Setup/carousel changes still require touching `screens.js` sections directly; tests for carousel render logic are not isolated to a module boundary | Complete extraction of setup/carousel renderers into `ui/screens/setup.js` (or equivalent) with `screens.js` limited to initialization wiring; boundary covered by unit tests |
