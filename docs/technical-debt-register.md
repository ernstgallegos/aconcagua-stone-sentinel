...
| `\|\|` numeric/object defaults in `screens.js` perception layer | Standardized 16 `\|\|` → `??` instances across `calculatePerceptionLatency`, `calculatePerception`, `getStageModifier`, `calculateBodyTolerance`, `applyAcclimatizationGain`, and `applyBivouacPenalty` to prevent zero-value coercion in gameplay-affecting calculations. | post-v1.5.1 audit (phase 4) |
