...
| Narrative bank i18n parity (NARRATIVES_ES only covers core action keys; many EN keys lack ES translations) | Web-v1 i18n maintainers | Low: ES players see EN fallback for secondary narrative events | v1.5.0 | Spanish players encounter English text for bivouac, dehydration, terrain-block, and other environmental narratives | Complete ES translations for all NARRATIVES keys; parity test verifying key coverage across both banks |
