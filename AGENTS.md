...
- Distinctive public-facing redesigns are more coherent when landing and active prototype share one token family update in the same commit (fonts, accent hierarchy, atmospheric backgrounds, and motion language), so the Play CTA does not create a stylistic quality drop between `/` and `prototype/web-v1`.
