...
- Full-surface aesthetic rebuilds are safer when landing and `web-v1` start from synchronized token-level typography/palette updates (`src/styles/public-tokens.css` + `prototype/web-v1/css/tokens.css`) before component polish; this prevents “beautiful landing, generic prototype” drift in the same release.
