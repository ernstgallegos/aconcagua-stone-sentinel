...
- Part 2 pre-threshold narrative updates stay stable when each beat is a dedicated linear screen ID wired into both `showScreen()` gating and hash deep-link allowlists; forgetting one ID silently breaks gated navigation and `&force=1` QA routes.
