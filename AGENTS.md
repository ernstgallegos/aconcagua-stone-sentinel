...
- Distinctive aesthetic passes are safer when landing and `web-v1` share one typography/token direction (same serif/sans/mono roles + aligned accent hierarchy) so the visual identity survives the Play CTA transition instead of resetting between surfaces.
