...
- Cross-surface visual overhauls are safer when each public surface (`/`, `md-viewer`, `web-v1`, archived viewer) consumes one shared semantic palette family (primary/secondary/accent + surface tiers) via tokens/vars first; this enables aggressive stylistic shifts without breaking existing screen/state logic.
