...
- Cross-surface visual redesigns are more coherent when landing and `web-v1` share the same token family update in one commit (font import + color tokens + key HUD surfaces), so users do not feel a style reset when transitioning from `/` to gameplay.
