...
- Public-site redesigns remain maintainable when the root landing keeps one integrated bilingual file (`index.html`) but enforces an editorial IA spine (atmospheric hero → manifesto → structure timeline → canonical materials), subtle single-purpose motion, and shared `data-doc-link` resolution so EN/ES document routing cannot drift.
