...
- Full-bleed hero refreshes are safer when the opening viewport is strictly limited to brand-first headline, one supporting sentence, one CTA cluster, and a single edge-to-edge mountain image; move release/status metadata below the fold to prevent dashboard-style visual noise.
