...
- Cross-surface redesign passes are safer when landing (`/`), markdown viewer, and `web-v1` token palettes are tuned together in one commit; isolated per-surface color edits quickly reintroduce visual drift.
