...
- Landing + web-v1 cohesion improved when both surfaces share the same editorial typography contract (`Fraunces` narrative, `Space Grotesk` UI, `IBM Plex Mono` telemetry) and the first viewport exposes an explicit “project pulse” block (flow/release/Part 2 scope) before secondary materials.
