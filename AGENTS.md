...
- Full visual-overhaul deliveries for public web surfaces are safer when implemented as self-contained HTML artifacts (`/index.html`, `/game.html`) that preserve typography/color/motion contracts without introducing external build coupling.
- Standalone redesign playables should keep explicit EP/BT telemetry visibility (even when simplified) so atmospheric UI improvements do not hide the mountain-authority systemic language expected by reviewers.
