...
- Full-bleed first-view compositions stay clearer when hero budgets are enforced literally (brand + one headline + one support line + CTA group + one dominant image) and metadata rows are pushed below the fold; this preserves mountain atmosphere without dashboard clutter.
