...
- Full-front-end makeover passes are safer when both `/index.html` and `prototype/web-v1` share the same hero grammar (full-bleed mountain plane, explicit brand lockup, one CTA cluster) so the transition from landing to playable surface feels like one product system.
