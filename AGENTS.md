...
- Full-surface visual overhauls are safest when implemented as token-first CSS overrides (`tokens/themes/layout/components/screens`) that preserve DOM IDs/window handlers; this enables aggressive UI shifts without breaking deterministic smoke and engine tests.
