...
- Disruptive landing redesigns become safer when each new section and animation hook is introduced with bilingual key parity in the same commit (`data-i18n` + `TRANSLATIONS`) to avoid polished visuals shipping with language regressions.
- Landing transformations align better with project intent when section architecture explicitly follows the curated art grammar (`cover` as threshold, `concept-art/curated` as atmospheric doctrine, `characters` as consequence scale) instead of generic marketing blocks.
