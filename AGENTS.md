...
- Multilingual startup UX requires startup/fatal copy to flow through `uiText`/language state; helper modules should accept localized copy payloads and `setLanguage()` should re-apply startup/fatal text to avoid EN-only regressions in ES mode.
