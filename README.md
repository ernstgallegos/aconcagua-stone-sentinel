...
- **Canonical index (`/`)**: now serves an alpine-editorial public landing page (asymmetric layout, atmospheric motion, bilingual EN/ES switch) with a primary CTA to `prototype/web-v1/index.html`.
...
> Note: v1.4 documentation and planning are now fully implemented through v1.4.8. For the current state and module map, see `docs/repo-truth.md` and the `[1.4.8]` CHANGELOG block.
...
## Prototype canonical status (v1.4.8 public state)

> **Canonical status (source-anchored):**
> - Live implementation status is tracked in `CHANGELOG.md` under [`[1.4.8]`](./CHANGELOG.md).
...
The canonical active prototype is **`prototype/web-v1` (v1.4.8 public state)**.

- `prototype/web-v1/`: active systemic prototype, node-to-node route, EP/BT/delta engine, and v1.4.8 modularized mechanics in public rollout.
  ...
  - Debrief analytics (`findTurningPoint`, `findPrimaryCause`, `buildReflectionPrompts`) are extracted to `ui/screens/debrief.js` as pure functions.
  - Utility functions (`formatMinutes`, `confidenceTier`, `resolveNavigationTarget`, etc.) are in `ui/helpers/screen-utils.js`.
